...
public class AnnotQualifierGroup {
    ...
    public AnnotQualifierGroup(ArrayList<AnnotationForTerm> annotForTermList) {
        if (null == annotForTermList) {
            return;
        }
        for (AnnotationForTerm aft: annotForTermList) {
            HashSet<Annotation> annotSet = aft.getAnnotSet();
            if (null == annotSet) {
                continue;
            }
